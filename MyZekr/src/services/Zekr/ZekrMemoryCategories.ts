...
export const ZekrAllCategories: ZekrCategory[] = [ZekrCategoryAfterNamaz, ZekrCategoryNamaz];

export function getZekrCategoryMemoryParentByParentUid(uid: string): ZekrCategory[] | undefined {
	let children = ZekrAllCategories.find((c) => c.uid === uid)?.children;

	let childrenCats = ZekrAllCategories.filter((c) => children?.includes(c.uid) === true);
	return childrenCats;
}

export function getZekrCategoryMemoryByUid(uid: string): ZekrCategory | undefined {
